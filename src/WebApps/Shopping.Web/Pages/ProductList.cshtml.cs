...
namespace Shopping.Web.Pages
{
        ...
        : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(string categorynName)
        {
            var response = await catalogService.GetProducts();

            CategoryList = response.Products.SelectMany(p => p.Category).Distinct();

            if (!string.IsNullOrEmpty(categorynName))
            {

                ProductList = response.Products.Where(p => p.Category.Contains(categorynName));
                SelectedCategory = categorynName;
            }
            else
            {
